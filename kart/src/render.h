 ...
 * member of Vehicle without either renderer needing to see the other's
 * internals: render_gl.c only touches gl_id, render_gx.c only touches
 * gx_handle (heap-allocated there; NULL here means "no texture").
 */
typedef struct {
    int          loaded;
    int          width, height;
