 ...
 * as a fallback tint for meshes with no bound texture (see model_load.h);
 ...
 * registers over the disc's own FST (see model_load.h/ASSET_ROOT), so this
 ...
 * fallback model, so the parser is written exactly once and shared by
 * all three.
 */
