 ...
 * placement tuning), plus the shared loader that builds one from a mesh
 * (OBJ or COLLADA, picked by file extension - see model_load.c) + PNG pair.
 * Used by characters (character.c) so the "load a mesh, recenter it,
 * auto-scale it, load its .png" logic lives in exactly one place.
 */
#ifndef MODEL_LOAD_H
#define MODEL_LOAD_H
...
/*
 * Loads a mesh (recentered so it sits on the ground and auto-scaled so its
 * largest dimension equals `target_length`) plus an optional PNG texture.
 ...
 * only flavor the diagnostic messages (e.g. "character"/"Racer One").
 */
